...
A complete library can be done using exclusivelly (\pfds). Formally proving most or all of their algorithms. Session types and \pfds can be succesfully implemented on mission critical financial problems, preserving the speed and accuracy of current methods. Offering the solid foundations of \fp and provide some advantajes as type sound concurrency. While being more modern, scalable, safe and coherent than current ones.\\

Using \pfds will likely yield many benefits, among them, will simplify parallelism  and concurrent computations. Because as \pfds are immutable locking is unnecesary. They will also benefit of lazines, and given that \pfds are persistent, no recalculation is necesary. \pfds also enables algebraic reasoning over the data sctuctures, wich given the sofistication on some calculus, is a very usefull feature.\\
