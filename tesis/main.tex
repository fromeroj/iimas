...
There are many libraries for financial calculation, most of them are proprietary, and since speed and accuracy play an important role specially in the valuation and trading areas, most of them are made in ``close to the metal'' languages such as c++. Another very important trait is correctness, but since it has been long thought that it is infeasible to achieve it, it has been marginalized to use unit testing, some model validation techniques and extensive debugging is made to reach certain level of correctness.\\

For some other problems such as stock valuation which in its very nature exhibits a very complex interdependent and concurrent behaviour, where real-time analysis is critical, specially today where high frequency trading (HFT) is the norm, having a valuation done faster or more accurately even if is just a $\mu s$ faster or a $\mu \pounds$ more accurate than that of a competitor can yield a vast and potentially insurmountable advantage. There are many problems with the current software, mostly due to the need for speed and craft nature of the development of realtime systems, creating a potpourri of techniques used but lacking a coherent approach.\\

On the other hand, functional programming (FP) and particularly \pfds are gaining traction given everyday more capable compilers and mode sophisticated techniques which allow to use effectively and reliably the everyday more common parallelism, distributed power which virtualisation and other ``cloud'' computing techniques are allowing, also lots of progress has been made in the field of formal proof systems which happen to be mature enough to create software formally proved correct in budget and timely. 
Session types have helped to specify and type data exchanges, having matured in its simple yet expressive elegance, allowing to describe and reason about concurrent systems guaranteeing communication safety, fidelity and type soundness.

Sadly most of the current libraries of \pfds are mostly used in domestic consumption and used by functional programming researchers and enthusiasts, and little has been leaked for use on other fields. 
